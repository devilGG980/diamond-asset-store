...
const Store: React.FC = () => {
  ...
  const [sortBy, setSortBy] = useState('featured'); // featured, downloads, newest
  ...
  // Dynamic SEO based on search and category - OPTIMIZED FOR FAST RANKING
  const getPageTitle = () => {
    if (searchQuery) {
      return `${searchQuery} Free Video Editing Assets | Download Now`;
    }
    if (selectedCategory !== 'All') {
      return `Free ${selectedCategory} Download | Professional Video Editing Assets`;
    }
    // FAST RANKING KEYWORDS: KD 10-12, High Volume (4200+/month)
    return 'Free Video Editing Assets Download | 500+ Transitions, Templates & Effects';
  };

  const getPageDescription = () => {
    if (searchQuery) {
      return `Download free ${searchQuery} video editing assets instantly. Professional transitions, templates & effects for Premiere Pro, After Effects & DaVinci Resolve. Join 1000+ creators. No watermark, no subscription.`;
    }
    if (selectedCategory !== 'All') {
      return `Download professional free ${selectedCategory.toLowerCase()} for video editing. 100% royalty-free, compatible with all editing software. Perfect for YouTube creators, TikTok makers & video editors. Instant access.`;
    }
    // OPTIMIZED for: "free video editing assets", "video editing resources free", "free video assets download"
    return 'Download 500+ free video editing assets instantly. Professional transitions, templates, backgrounds & effects for all editing software. No watermark, no signup. Used by 1000+ creators worldwide. Fast ranking keywords optimized for maximum visibility.';
  };

  const getKeywords = () => {
    // FAST RANKING KEYWORDS (KD 6-12)
    const baseKeywords = [
      'free video editing assets',      // KD: 11, Volume: 4,200
      'video editing resources free',   // KD: 10, Volume: 2,600 - EASIEST
      'free video assets download',     // KD: 12, Volume: 3,800
      'best free video assets',         // KD: 12, Volume: 2,100
      'free video editing resources',   // KD: 10, Volume: 2,600
    ];
    if (searchQuery) {
      baseKeywords.push(`free ${searchQuery}`, `${searchQuery} assets`, `best free ${searchQuery}`);
    }
    if (selectedCategory !== 'All') {
      baseKeywords.push(
        `free ${selectedCategory.toLowerCase()}`,
        `${selectedCategory.toLowerCase()} free download`,
        `best free ${selectedCategory.toLowerCase()} for video editing`
      );
    }
    return baseKeywords;
  };
  ...
  const storeStructuredData = {
    ...
    "url": `https://editorvault.web.app/store${searchQuery ? `?search=${encodeURIComponent(searchQuery)}` : ''}${selectedCategory !== 'All' ? `?category=${encodeURIComponent(selectedCategory)}` : ''}`,
    "mainEntity": {
      "@type": "ItemList",
      ...
      "itemListElement": filteredAssets.slice(0, 10).map((asset, index) => ({
        ...
        "item": {
          ...
          "image": asset.thumbnail ? `https://editorvault.web.app${asset.thumbnail}` : "https://editorvault.web.app/logo.svg",
          "url": `https://editorvault.web.app/asset/${asset.id}`,
      ...
      "itemListElement": [
        {
          ...
          "position": 1,
          "name": "Home",
          "item": "https://editorvault.web.app"
        },
        {
          "@type": "ListItem",
          "position": 2,
          "name": "Store",
          "item": "https://editorvault.web.app/store"
        }
      ]
    }
  };
    ...
    <>
      ...
      <div className="min-h-screen p-2 sm:p-4 safe-area-top">
        <div className="max-w-7xl mx-auto">
          ...
          >
            {/* OPTIMIZED H1 for fast ranking - Includes primary keyword: "free video editing assets" */}
            <h1 className="text-3xl sm:text-5xl font-bold text-gradient mb-3 sm:mb-4">
              {selectedCategory !== 'All'
                ? `Free ${selectedCategory} Assets Download - Professional Video Editing Resources`
                : 'Free Video Editing Assets Download - 500+ Transitions, Templates & Effects'}
            </h1>
            <p className="text-lg sm:text-xl text-gray-400 mb-6 sm:mb-8 px-2">
              Download <strong>free video editing assets</strong> including professional <Link to="/store?category=Transitions" className="text-cyan-300 hover:text-cyan-400 underline">transitions</Link>, <Link to="/store?category=Templates" className="text-cyan-300 hover:text-cyan-400 underline">templates</Link>, <Link to="/store?category=Backgrounds" className="text-cyan-300 hover:text-cyan-400 underline">backgrounds</Link> & <Link to="/store?category=Motion" className="text-cyan-300 hover:text-cyan-400 underline">motion graphics</Link>. Compatible with Premiere Pro, After Effects & DaVinci Resolve. <strong>No watermark. No subscription.</strong>
            </p>
              ...
              <div className="flex flex-wrap gap-2">
                {categories.map((category) => (
                  <button
                    ...
                    className={`px-3 sm:px-4 py-2 rounded-lg transition-all text-xs sm:text-sm touch-target ${selectedCategory === category
                      ? 'bg-white text-black'
                    ...
                    <div className="relative mb-4 bg-gray-800 rounded-lg overflow-hidden aspect-video">
                      {hoveredAsset === asset.id && asset.previewVideo ? (
                        ...
                        <div className="flex items-center justify-center h-full group-hover:scale-110 transition-transform duration-300">
                          {asset.thumbnail && (asset.thumbnail.startsWith('data:') || asset.thumbnail.startsWith('/') || asset.thumbnail.startsWith('http')) ? (
                            <img
            ...
            <div className="grid md:grid-cols-2 gap-8">
              <div>
                ...
                <ul className="text-gray-300 space-y-2">
                  ...
                  <li>• <strong>Instant Download:</strong> Get your assets immediately</li>
